...
/*------------------------------------------------------------- 
  Lab 9 – Plot 2
  Relationship Between  Time Spent Outside and Daily Reflection
...
// Filter out stay-home days to focus on meaningful reflection scores
let plotData = dailyData.filter(d => d.hoursOutside > 0);

/* Explanation:
   filter() removes days with 0 hours outside to focus on days with actual experience.
*/

// Sort data by mealsPurchased so larger circles are drawn first
plotData.sort((a,b) => b.mealsPurchased - a.mealsPurchased);

/* Explanation:
   sort() ensures larger bubbles are behind smaller ones,
   preventing overlap from hiding data points and improving readability.
*/
/* === Scales === */
